...
if [ "${PACKAGE}" == "master" ] || [ "${PACKAGE}" == "aws-fpfis-repo" ]; then
  PACKAGE="aws-fpfis-repo"
  SPECFILE="aws-fpfis-repo-${EL}.spec"
else
...
docker ${CONTAINER}  /usr/bin/mock -r fpfis-${EL}-x86_64 --spec=/mock/build/SPECS/${SPECFILE} --sources=/mock/build/SOURCES/${PACKAGE} --resultdir=/mock/build/SRPMS --buildsrpm

echo "Building RPM ..."

docker ${CONTAINER} /usr/bin/mock --clean -r fpfis-${EL}-x86_64  -D "dist .${DIST}" --resultdir=/mock/build/RPMS --rebuild /mock/build/SRPMS/$(ls -1utr SRPMS/|grep ^${PACKAGE}-.*src\.rpm$|head -1)
